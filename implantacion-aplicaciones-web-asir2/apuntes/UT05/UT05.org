...
if($result) {
    ...
    echo "Código de error mysql: $conexion->errno";
    echo "Mensaje de error mysql: $conexion->error";
}
#+end_src
...
- Existe la tentación de construir /strings/ con las sentencias
#+begin_src php
$id = $_GET["id"];
...
- Es la única forma segura de pasar cadenas y números a una sentencia SQL
  - [[https://www.php.net/manual/en/mysqli.real-escape-string.php][=mysqli_real_escape_string=]] puede tener [[https://stackoverflow.com/a/15786451][problemas]]
- Pasos:
