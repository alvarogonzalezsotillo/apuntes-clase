...
\textattachfile{/home/alvaro/apuntes-clase/implantacion-aplicaciones-web-asir2/apuntes/UT04/UT04-practica-php.org}{} \vspace{-1em}
...
\end{itemize}
\section{Mazmorra tipo \emph{Mario Bros} (2 puntos)}
...
\item Se garantiza que las imágenes del \emph{array} son correctas
\item Cada celda del array contendrá un \emph{string}, indicado el \emph{tile} que debe ponerse en la mazmorra
\item La función creará una página web con las imágenes que dibujan la mazmorra. \href{https://alvarogonzalezsotillo.github.io/apuntes-clase/implantacion-aplicaciones-web-asir2/apuntes/UT04/tiles/tiles.zip}{Las imágenes se pueden encontrar en este fichero ZIP}
\item Se puede usar \texttt{table}, \texttt{div}, \texttt{flex}\ldots{}
\end{itemize}

\attachfile{./tiles/tiles.zip}

\begin{lstlisting}[language=php,caption={Ejemplo de uso de \texttt{mazmorra()}},captionpos=b,numbers=none]
$tiles = [
    ["E",  "E",   "E", "E"],
    ["E",  "ETE", "E", "E"],
    ["ET", "TIE", "E", "ET"],
...
\item Para alinearlo, se puede utilizar \texttt{table}, CSS con \texttt{grid},\ldots{}
\end{itemize}
\end{itemize}


\begin{lstlisting}[language=php,caption={Ejemplo de uso de \texttt{formulario()}},captionpos=b,numbers=none]
...
\clearpage
\section{Calculadora simple (5 puntos)}
...
\end{itemize}
\item Al enviar el formulario, se mostrará el resultado de la operación (10\%). El formulario volverá a mostrar los últimos valores introducidos (10\%)
\item Si la operación no es posible, en vez del resultado se mostrará \texttt{Error} (50\%)
\item Si es la primera vez que se ejecuta, aparecerán los valores vacíos, y \texttt{+} seleccionado (10\%)
\item Para la operación módulo:
\begin{itemize}
\item Los operandos se aproximarán a enteros (10\%)
\item En vez de mostrar los operandos originales, se mostrarán los aproximados a entero (10\%).
\end{itemize}
\end{itemize}
(\href{https://443s--main--p3--admin.coder.iesavellaneda.es/public-html/practica01/calculadora.php}{ver ejemplo})
\begin{figure}[htbp]
\centering
...
\end{itemize}
\item Para probar los dos primeros ejercicios, el profesor subirá un fichero como el siguiente al Apache del alumno
\end{itemize}


\begin{lstlisting}[language=php,caption={Ejemplo de fichero de prueba},captionpos=b,numbers=none]
<?php
