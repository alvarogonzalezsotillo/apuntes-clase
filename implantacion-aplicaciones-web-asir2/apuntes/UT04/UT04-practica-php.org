...
- Mostrará un formulario con un número con dos decimales, un selector con operaciones matemáticas, y un segundo número con dos decimales.
  - =+=, =-=, =%=, =/=, =*=
- Al enviar el formulario, se mostrará el resultado de la operación. El formulario volverá a mostrar los últimos valores introducidos.
- Si la operación no es posible, en vez del resultado se mostrará =Error=
- Si es la primera vez que se ejecuta, aparecerán los valores vacíos, y =+= seleccionado.  
...
- Para probar los dos primeros ejercicios, el profesor incluirá un fichero como el siguiente, y lo ejecutará en en Apache del alumno
