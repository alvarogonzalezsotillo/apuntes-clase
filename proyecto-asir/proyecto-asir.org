...
digraph arquitectura {
  ...
  vpn -> proxy  [label=" acceso SSH", style=dashed, color="#7c2f4a"];
  vpn -> apache [label=" acceso SSH", style=dashed, color="#7c2f4a"];
  vpn -> db     [label=" acceso SSH", style=dashed, color="#7c2f4a"];
  vpn -> pma    [label=" backend pma.asir.test", style=dashed, color="#7c2f4a"];
  vpn -> dns    [label=" resolucion asir.test", style=dashed, color="#7c2f4a"];
}
#+end_src
...
| 80 y 443     | =proxy=  | WordPress (=www.asir.test=) y PHPMyAdmin (=pma.asir.test=)                 |
...
Se emplea el dominio reservado para pruebas =asir.test= (RFC 6761). Registros requeridos:
 ...
 - Alias =www.asir.test= (CNAME de =proxy=).

** Estructura del repositorio :noexport:
...
│   ├── named.conf.options
│   ├── named.conf.local
│   └── zones/               # zona directa e inversas
└── scripts/
    ├── install.sh           # prepara WordPress, .env, directorios
...
- Desplegar un servidor LAMP (Linux + Apache + MySQL + PHP) en /tres contenedores/:
  =apache= (Apache con PHP), =db= (MySQL) y =phpmyadmin= (gestión de =db=).
- Los tres contenedores deben formar parte de una red Docker propia (=red-interna=)
...
- Debe almacenar un registro de nombre por cada contenedor del proyecto y los registros de resolución inversa (PTR).
...
4. Definir la zona directa =asir.test= con un A por contenedor (=dns=, =proxy=, =vpn=, =apache=, =db=, =pma=) y los CNAME =phpmyadmin= → =pma= y =www= → =proxy=.
...
** Tareas
1. En =scripts/install.sh=: descargar =https://wordpress.org/latest.tar.gz= y
   extraerlo en =apache/www=. No sobrescribir si ya existe =wp-config.php=.
2. Generar =wp-config.php= con =DB_NAME=wordpress=, =DB_USER=wp_user=,
   =DB_PASSWORD=wp_pass_2026=, =DB_HOST=db= y las 8 /salts/ generadas con
   =openssl rand -base64 48=.
3. Levantar el escenario y completar el asistente web apuntando el navegador a
   =http://localhost:8080= (en esta fase todavía sin proxy).
4. Crear un artículo de prueba y comprobar que tras
   =docker compose restart db= el artículo sigue existiendo (persistencia en
   =db/data=).
...
** Enunciado
- Colocar un contenedor =proxy= (nginx) como puerta de entrada única al servidor
  Apache y al servidor PHPMyAdmin.
- El proxy usará dos nombres virtuales: =www.asir.test= → =apache:80= y
  =pma.asir.test= → =phpmyadmin:80=.
- A partir de esta fase los clientes acceden únicamente a través del proxy; se
  retiran las publicaciones =8080= y =8081=.
...
6. Comprobar desde el host añadiendo una entrada estática al DNS del sistema (o usando =--resolve= con =curl=) y después mediante el DNS de la fase 2.
...
** Comprobaciones
  ...
  #+end_src
- Abriendo =http://www.asir.test= y =http://pma.asir.test= en el navegador (con
