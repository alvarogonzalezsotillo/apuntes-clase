...
Esta práctica forma parte del RA3 y el RA5.
...
\item Todos los empleados acceden a internet a través del mismo router (puedes representar internet por un servidor con la ip \texttt{8.8.8.8}). \textbf{No} es necesario que se haga \enlace{https://www.cisco.com/c/en/us/support/docs/ip/network-address-translation-nat/13772-12.html}{NAT}.
...
\item Incluye al menos:
  ...
  \item Los routers serán del modelo 1941  
  \end{itemize}
\item Los equipos estarán en subredes de \texttt{10.N.0.0/16} (siendo \texttt{N} el número asignado al alumno usado en otras prácticas de Packetracer).
\item Además de los pedidos en el enunciado, pueden añadirse los switches y routers adicionales que se necesiten.  
\end{itemize}

\section{Opcional: IPv6}
Se desea que, además de las direcciones IPv4, internamente se pueda utilizar el protocolo IPv6. La red a repartir será la \texttt{fc00:N::/32}.
...
\item Se entregará un fichero PKT que pueda abrirse con la versión 8.2.2.0400 de Packet Tracer (la de clase).
...
\item Que los departamentos puedan comunicarse a través de la capa 3 (\textbf{2.5} puntos)
...
\item \textbf{No se valorará} el ejercicio
  \begin{itemize}
  \item Si los equipos pierden la configuración al reiniciar.
  \item Si se usa \texttt{\enlace{https://es.wikipedia.org/wiki/Proxy\_ARP}{ARP Proxy}}
