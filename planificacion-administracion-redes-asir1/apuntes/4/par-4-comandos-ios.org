...
Nota: Se puede usar =do= para ejecutar un comando de /enabled/ en el modo de configuración.    
...
El puerto del otro switch cambiará automáticamente, a no ser que esté configurado como =switchport nonegotiate=
#+begin_src lisp
...
Se añaden a un mismo =channel-group= todos los enlaces conectados al mismo switch.
...
Determinar la prioridad de un enlace
...
#+begin_src lisp
R1(config)# router ospf 10
R1(config-router)# network 10.1.0.0 255.255.0.0 area 0
#+end_src


Incluir la red de un interfaz
...
Información OSPF de una interfaz. Información del entorno OSPF
...
Costo manual de una interfaz y costo de referencia 
...
#+begin_src lisp
R1(config)# ip route 0.0.0.0 0.0.0.0 1.2.3.4
R1(config)# router ospf 10
