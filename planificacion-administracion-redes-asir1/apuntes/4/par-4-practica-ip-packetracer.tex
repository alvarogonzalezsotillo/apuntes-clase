...
\item Red de routers: Cada planta tendrá un \textit{router}. Los routers se unirán entre sí en otra subred de la red original, con capacidad para 25 routers.
\end{itemize}
...
\begin{itemize}
\item Cada planta tendrá un \emph{router}, su \emph{switch} (sistema horizontal), el primer puesto de trabajo y el último puesto de trabajo
\item Los \textit{routers} de las plantas se unirán por un switch (sistema vertical).
\item Habrá comunicación a nivel IP (\texttt{ping})
\begin{itemize}
\item entre todos los puestos de trabajo de una planta
\item entre todos los \textit{routers}, usando la red de \textit{routers}
\item entre todos los puestos de trabajo y todos los \textit{routers}
\end{itemize}
\item No se considera un error que no se pueda hacer \texttt{ping} entre puestos de trabajo de plantas distintas
\end{itemize}
