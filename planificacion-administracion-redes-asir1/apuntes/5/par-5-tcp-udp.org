...
- Se llama puerto a la dirección de nivel de transporte en
  - TCP
  - UDP
- La asignación de puertos se realizaba según el [[http://www.ietf.org/rfc/rfc1700.txt?number=1700][RFC 1700]]. Ahora es un [[https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.xhtml][registro de la IANA]]
...
[[https://www.ssh.com/academy/ssh/port][¿Cómo se puede registrar un puerto?]]
** Comando =netstat=
- Informa de 
...
* SSL
- Muchos protocolos basados en TCP han añadido SSL
  - =HTTP= ➔ =HTTPS=
  - =IMAP= ➔ =IMAPS=
- SSL aporta encriptación (sería parte de la capa de presentación)
- En ese caso, en vez de =netcat= usaremos =openssl=
  #+begin_src bash
  openssl s_client -quiet -connect www.debian.org:443
  #+end_src
- Ejemplo completo:
  #+begin_src bash
  printf "GET / HTTP/1.1\r\nHost: www.debian.org\r\n\r\n" \
