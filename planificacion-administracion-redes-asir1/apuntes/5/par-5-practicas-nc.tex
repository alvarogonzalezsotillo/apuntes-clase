...
\end{listadoshell}
  ...
  \item Envía el fichero a un compañero usando \texttt{netcat}. Mide el tiempo total de transmisión con el comando \texttt{time}, y calcula la velocidad de transferencia.
...
\begin{homeworkProblem}[: \imenu{Simulación de cliente web (2 puntos)}]
  ...
  \begin{itemize}
  \item Ejecuta NetCat conectándote al puerto 80 de \url{api.qrserver.com}
  \item Tras la conexión, introducir
    \begin{cuadrito}
      \texttt{GET /v1/create-qr-code/?data=\textit{eltextoquequeramosverenelqr} HTTP/1.1} \\
      \texttt{Host: api.qrserver.com} \\
      \\
      \\
  ...
  \item \textit{eltextoquequeramosverenelqr} tiene que estar en el formato \enlace{https://www.urlencoder.io/}{URLencode}
  \item Fedora con \texttt{ncat}: Si no se consigue ninguna respuesta, puede ser que se corte la conexión antes de recibirla. Prueba con \texttt{--no-shutdown --idle-timeout 2} para que \texttt{ncat} siga esperando tras el fin de su entrada estándar.
  \item En la respuesta están las cabeceras y la imagen del código QR. Tendrás que quitar las cabeceras, por ejemplo con \texttt{tail}.
...
Incluye el código QR generado en el trabajo, junto con los comandos o el procedimiento seguido para separar la imagen PNG de las cabeceras


  
\begin{comment}

curl https://api.forismatic.com/api/1.0/\?method\=getQuote\&format\=json\&lang\=en
{"quoteText":"Be faithful in small things because it is in them that your strength lies.  ", "quoteAuthor":"Mother Teresa ", "senderName":"", "senderLink":"", "quoteLink":"http://forismatic.com/en/bde669ecf1/"}%      
