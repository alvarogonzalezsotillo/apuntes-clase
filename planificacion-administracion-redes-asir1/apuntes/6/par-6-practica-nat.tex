...
\item  \textbf{A}: Máquina virtual Linux (recomendado Debian)
  ...
  \item Tendrá una tarjeta de red \textit{bridged} con la dirección \texttt{192.168.255.N/24} (siendo \texttt{N} tu número de ordenador)
    \begin{itemize}
    \item \url{https://docs.google.com/spreadsheets/d/1z1B07ee3VjmYwtd1i0wCBRPnjZS00oExbGXNDlNa93E/edit?gid=0#gid=0}.
    \end{itemize}
...
\begin{homeworkProblem}[: Activa el NAT en la máquina A (2 puntos)]
  ...
  \end{itemize}
          ...
          {\vspace{-1cm}\noindent\fontsize{.1}{.1}\selectfont Las direcciones IP serán dinámicas, distribuidas por un servidor DHCP local, que deberás instalar y configurar para que reparta el rango 172.16.4.0/23, dejando la 172.16.8.1 para el gateway\vspace{-.5cm}}
  
\end{homeworkProblem}

\begin{homeworkProblem}[: Conéctate a B por medio de escritorio remoto, desde tu PC físico (1 punto)]

          {\vspace{-1cm}\noindent\fontsize{.1}{.1}\selectfont Los usuarios estarán definidos en el Active Directory. Una política definida con Powershell dará acceso solo a la unidad organizativa de nombre ``Acceso remoto''. Describe los pasos seguidos para instalar Active Directory y la creación de la unidad organizativa \vspace{-.5cm}}
...
\begin{homeworkProblem}[: Convierte a B en un servidor de ficheros (carpetas compartidas) ]
            ...
            {\vspace{-1cm}\noindent\fontsize{.1}{.1}\selectfont Automatiza con Powershell todas las tareas de este ejercicio, para poder repetirlas en otro ordenador fácilmente, incluyendo configuracioń de Active Directory, Firewall, políticas locales y cambios en el registro de windows. Opcionalmente, se puede usar bat en vez de powershell. Incluye el script de automatización. \vspace{-.5cm}}

  \begin{itemize}
  \item Abre los puertos necesarios en \textbf{A} para que pueda accederse a \textbf{B} como servidor de ficheros (\enlace{https://petri.com/smb-port-445-139-138-137/}{ayuda}).
...
\begin{homeworkProblem}[: Haz que A y C puedan ser administrados mediante SSH desde la red del aula ]
          ...
          {\vspace{-1cm}\noindent\fontsize{.1}{.1}\selectfont Usa el fichero /etc/sshd/config para realizar la persistencia de los cambios, y crea un servicio de systemd específica para este servidor ssh, que escuchará en el puerto 222. Incluye los ficheros necesarios para el servicio de systemd.\vspace{-.5cm}}
  ...
  Para que pueda comprobarse, cambia las claves del servidor SSH de uno de los dos servidores Linux:

  \begin{listadoshell}{Reiniciar claves de servidor SSH}{lst:claves-ssh}
  rm /etc/ssh/ssh_*_key*
  ssh-keygen -A
...
\begin{homeworkProblem}[: Comprueba el NAT con \texttt{tcpdump} ]
          ...
          {\vspace{-1cm}\noindent\fontsize{.1}{.1}\selectfont la herramienta tcpdump se instalará compilando su código fuente, en https://github.com/the-tcpdump-group/tcpdump. Incluye los pasos para la compilación.\vspace{-.5cm}}
