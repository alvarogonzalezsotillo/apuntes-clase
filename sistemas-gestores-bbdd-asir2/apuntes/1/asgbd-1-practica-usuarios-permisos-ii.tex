...
\textattachfile{/datos-luks/datos-alvaro/apuntes-clase/sistemas-gestores-bbdd-asir2/apuntes/1/asgbd-1-practica-usuarios-permisos-ii.org}{} \vspace{-1em}
...
\item NEREAGIL1
\item JAVIERGOMEZ24
\item ANDRESGONZALEZ52
\item DANIELGONZALEZ70
\item LUIGIHALAS
\item HUGOHERMOSILLA1
\item JUANJARILLO1
\item SERGIOLOPEZVENTURA
\item MONICAMARTINEZ26
\item JPMC50
\item HECTORMORENO41
\item PABLONOMBELA
\item ASIERPARDO
\item KRISTINAPELESHOK
\item MARIOTSVETELINOV
\item MIGUELRAMOSDEDIEGO
\item MIGUELREYES1
\item MIGUELROMAN9
\item OSKARSAJEK
\item VERONICAVILLASECA
\item DANIELZUMARRAGA
\item ALVAROGONZALEZ
\end{itemize}
\end{multicols}
...
\begin{itemize}
\item Por ejemplo, el usuario \texttt{alvarogonzalez} debería poder ejecutar \texttt{SELECT * FROM CIRCUITOS}, puesto que habrás creado un sinónimo del tipo \texttt{CREATE PUBLIC SYNONYM CIRCUITOS FOR MIUSUARIO.CIRCUITOS}.
\end{itemize}
\end{itemize}

\begin{Aviso}
Son muchos alumnos. Agrúpalos de algún modo. (40\%)
\end{Aviso}
...
\item Da permisos al resto de alumnos para poder leer y actualizar este campo, con una orden \texttt{GRANT} (20\%)
\item Pide a algún otro compañero que compruebe que funciona
\item Pide al profesor que compruebe que funciona.
\end{itemize}
\section{\texttt{PROFESOR} con permisos especiales}
