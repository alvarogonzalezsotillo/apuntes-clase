...
| Lectura no repetible | /Repeatable read/ | Un usuario lee menos filas (o filas cambiadas) en =select= sucesivas dentro de la misma transacción |
| Fila fantasma        | /Phantom read/    | Un usuario lee más filas en =select= sucesivas  dentro de la misma transacción                       |
...
- Bloqueo de una [[https://docs.oracle.com/cd/B19306_01/server.102/b14200/statements_9015.htm][tabla completa]]
  ...
  - también puede bloquear otras a partir de las /foreign keys/  
...
and s.sid = EL_SID_DEL_BLOQUEADO
order by piece;
#+end_src
...
** Tablas involucradas
#+BEGIN_SRC dot :file ./media/tablas-bloqueos.png :exports results :cmd dot :cmdline -T png
digraph {
  node[ shape="box"]
  graph [pad="0.5", nodesep="1", ranksep="1"];
  edge[fontname="courier", fontsize="8", color="darkgray"]
  
  splines=true
  rankdir=TD

   LOCK [label="v_$lock"]
   LOCKED_OBJECT [label="v_$locked_object"]
   PROCESS [label="v_$process"]
   SESSION [label="v_$session"]
   SQLTEXT [label="v_$sqltext"]

   LOCKED_OBJECT -> LOCK[label="object_id"]
   LOCK -> LOCKED_OBJECT[label="id1"]


   SESSION -> SESSION[taillabel="blocking_session" headlabel="sid"]
   LOCKED_OBJECT -> SESSION [headlabel="sid" taillabel="session_id"]
   SESSION -> PROCESS [taillabel="paddr" headlabel="addr"]
   SESSION -> SQLTEXT [taillabel="sql_address" headlabel="address"]
   LOCK  -> SESSION [headlabel="sid" taillabel="sid"]
   LOCK -> DBA_OBJECTS  [taillabel="id1" headlabel="object_id"]
   LOCK -> LOCK  [taillabel="id1 id2" headlabel="id1 id2"]
   LOCKED_OBJECT -> DBA_OBJECTS[taillabel="object_id" headlabel="object_id"]
...
#+RESULTS:
[[file:./media/tablas-bloqueos.png]]
...
- Importante:
  - La columna ~v$process.spid~ es del proceso intermedio entre el cliente (/application code/) y el servidor (/system global area/)
  - La columna ~v$session.process~ es del proceso cliente, que está en la máquina cliente, no en el servidor
