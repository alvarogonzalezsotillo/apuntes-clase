...
cambiar_estado_instancia(){
    ...
    if instancia_encendida
    then
	apagar_instancia | espera_stdin "Apagando instancia"
    else
	encender_instancia | espera_stdin "Encendiendo instancia"
    fi
}
...
espera_stdin(){
    ...
    do
        ...
        if [ $SECONDS -ge 100 ]
        then
            SECONDS=$((99-RANDOM%10))
        fi
        printf 'XXX\n%d\n%s: %s\n' $SECONDS "$MESSAGE" "$LINE"
        
    done | dialog --gauge "$MESSAGE" 6 70 0    
}

espera(){
    local WAIT_CONDITION_CMD="$1"
    local MESSAGE="$2"
    ...
    delay(){
        ...
        do
            ...
            sleep 1s
            printf "XXX\n%d\nHan transcurrido %d segundos\n" $SECONDS $SECONDS
        done
        echo 100
    }

    delay "$WAIT_CONDITION_CMD" | dialog --gauge "$MESSAGE" 6 60 0
}
...
ejecutar_menu_tradicional()(
    PS3="Elige una opción:"
    while [ ! "$REPLY" = "3" ]
    do
        printf "\n\n\n\n"
        select OPT in "$(listener_mensaje_opcion)" "$(instancia_mensaje_opcion)" "Salir"
        do
            echo has elegido "$OPT" que es el número "$REPLY"
            case "$REPLY" in
                "1")
                    cambiar_estado_listener
                    break
...
#espera "[ ! -f findeespera ]" "Prueba de espera"
#espera_stdin "Prueba de espera"
#exit
