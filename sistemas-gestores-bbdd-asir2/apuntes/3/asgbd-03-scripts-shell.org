...
** Avanzado: usar gmail de /smarthost/
Esto no usa el servidor de correo local, pero hay que poner la contraseña en plano, y usar una [[https://www.systutorials.com/sending-email-from-mailx-command-in-linux-using-gmails-smtp/][App Password]]
#+begin_src bash
...
set smtp-auth-password=<quitado>
set from="XXXXX <XXXXXXXX@gmail.com>"
#+end_src

#+begin_src bash
echo hola | mailx -S "config=/home/alumno/.mailrc" -s "asunto" direccion@gmail.com
#+end_src
...
dialogo(){
    whiptail --title "Menu" --menu "Elige una opción" 12 50 5 \
...
casi_pantalla_completa(){
    stty size | awk '{$1-=4;$2-=4}1'
}

casi_ancho_completo(){
    echo $(stty size | cut -d ' ' -f 2) - 4 | bc
}

ejecuta_opcion(){
    case $1 in
        1)
            whiptail --title "Host name" --msgbox "$(hostname)" 9 $(casi_ancho_completo)
            ;;
        2)
            whiptail --title "Direcciones IP" --msgbox "$(hostname -I)" 9 $(casi_ancho_completo)
            ;;
        3)
            whiptail --title "Uso de disco" --msgbox "$(df | grep /dev | grep -v tmpfs)" $(casi_pantalla_completa)
            ;;
        4)
            whiptail --title "Usuarios con sesión" --msgbox "$(who)" $(casi_pantalla_completa)
            ;;
        5)
            echo "Saliendo"
            ;;
...
while [ "$respuesta" ] && [ "$respuesta" -ne 5 ]
do
...
- =bash= procesa los siguientes ficheros:
  - Si es un nuevo /login/: =/etc/profile= primero, luego el primero que exista de =~/.bash_profile=, =~/.bash_login= o =~/.profile=
  - Si es una /shell/ interactiva (no /login/): =/etc/bash.bashrc= (Debian) o =/etc/bashrc= (CentOS/RHEL), luego =~/.bashrc=
...
- Para que funcionen sin sesión activa, se necesita =loginctl enable-linger=
