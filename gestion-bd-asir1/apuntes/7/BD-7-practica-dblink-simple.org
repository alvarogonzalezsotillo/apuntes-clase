...
#+LATEX_HEADER: \lhead{Registro civil}
...
#+begin_src sql
create or replace view v_clientes(idcliente,nombre,sede) as
select ...;

create or replace view v_pedidos(idpedido,idcliente,importe,fechaeentrega) as
select ...;
#+end_src
...
| ~idpedido~     | ~NUMBER(10)~    | El identificador de pedido  |
| ~fechaentrega~ | ~DATE~          | Fecha de entrega del pedido |
| ~importe~      | ~NUMBER(10,2)~  | Importe del pedido          |
