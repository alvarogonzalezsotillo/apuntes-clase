...
** Identificador del curso que más alumnos pueden pedir, porque tiene más oferta de plazas
#+begin_src sql
select course_no from section
group by course_no
having sum(capacity) = (
  select max(sum(capacity)) from section group by course_no
)
#+end_src

** Lista de profesores y calle donde viven, sin número
#+begin_src sql
select 
  first_name, 
  last_name, 
  substr(street_address,instr(street_address,' ')+1)
from instructor
#+end_src
