...
- Diagramas de comportamiento (secuencia, actividad, estados)
...
class Almacen {
  ...
  + ajustaStockVenta(Venta)
  + ajustaStockEntrada(Entrada)
}
...
rectangle "Máquina Expendedora" {
  ...
  (Dar de alta producto) --> (Actualizar inventario) : <<extend>>
  (Dar de baja producto) --> (Actualizar inventario) : <<extend>>
...
rectangle Sistema {
  ...
  Gestor -- (Modificar documentos de otros)

  Administrador -- (Dar de baja usuario)
  Administrador -- (Modificar datos de usuario)
  Administrador -- (Borrar documentos)

  (Modificar documentos de otros) .> (Ver documentos de otros) : <<include>>
  (Sugerir cambios) .> (Modificar documentos de otros) : <<extend>>
  (Borrar documentos) .> (Modificar documentos de otros) : <<include>>
}
...
rectangle Sistema {
    ...
    (Elevar incidencia) --> (Abrir incidencia) : <<include>>
...
*** Solución 1
#+begin_src plantuml :file media/foro-1.svg
@startuml
left to right direction
...
rectangle Foro {
    Usuario -- (Registrarse)
    (Registrarse) <-- (Validar con SMS) : <<extends>>
    (Registrarse) <-- (Validar con email) : <<extends>>

    Administrador -- (Promover a escritor)    

    Usuario -- (Responder hilo)
    Escritor  -- (Crear hilo)

    Administrador  -- (Aceptar registro)
...
#+RESULTS:
[[file:media/foro-1.svg]]

*** Solución 2
#+begin_src plantuml :file media/foro-2.svg
@startuml
left to right direction
...
rectangle Foro {
    ...
    (Banear usuario) --> (Gestionar usuarios) : <<extends>>
    (Promover a escritor) --> (Gestionar usuarios) : <<extends>>
    (Consultar datos) --> (Gestionar usuarios) : <<extends>>
    (Aceptar registro) --> (Gestionar usuarios) : <<extends>>

    (Banear usuario) --> (Consultar datos) : <<includes>>    
    (Aceptar registro) --> (Consultar datos) : <<includes>>    
    (Promover a escritor) --> (Consultar datos) : <<includes>>    
...
*** solución :noexport:
#+begin_src plantuml :file ./media/almacen-refrigerado-1.svg
@startuml
start

:Recibir orden de salida;

:Separar en productos refrigerados y no refrigerados;

fork
  :Localizar productos refrigerados;
  :Empaquetar productos refrigerados;
fork again
  :Localizar productos no refrigerados;
  :Empaquetar productos no refrigerados;
end fork

if (¿Falta algún producto?) then (Sí)
  :Crear orden de reposición al proveedor;
  :Esperar llegada del producto;
endif

:Sacar productos del almacén;
...
#+RESULTS:
[[file:./media/almacen-refrigerado-1.svg]]

#+begin_src plantuml :file ./media/almacen-refrigerado-2.svg
@startuml
start
...
#+RESULTS:
[[file:./media/almacen-refrigerado-2.svg]]
...
*** solución :noexport:
#+begin_src plantuml :file media/busqueda-de-vuelos.svg
@startuml
start

repeat
  :Buscar vuelo (origen y destino);
  ...
  fork again
    :Consultar Proveedor 2;
  fork again
    :Consultar Mayorista 1;
  fork again
    :Consultar Mayorista 2;
  end fork

  :Contactar con el cliente\npara comunicar ofertas;

  repeat
    if (¿Cliente elige oferta?) then (Sí)
      :Contactar con proveedor\npara reservar vuelo;

      if (¿Oferta disponible?) then (Sí)
        :Confirmar reserva;
        stop
      else (No)
---        :Informar indisponibilidad;
  ...
  repeat while (¿Elegir otra oferta?) is (Sí)

repeat while (¿Reiniciar búsqueda?) is (Sí)
stop
...
#+RESULTS:
[[file:media/busqueda-de-vuelos.svg]]
...
** ejemplos :noexport:
busca ejemplos-uml-vida-real.org
...
** Ejemplo
[[https://www.madrid.org/bvirtual/BVCM016109.pdf][Manual de permisos y licencias]]
* Referencias
